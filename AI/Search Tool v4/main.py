...
def readPlanAndCreate():
    ...
    p = createProblem(parameters) # 파라미터 딕셔너리로 할꾸야
...
def createProblem(parameters): ###
    ...
    p.setVariables(parameters) #파라미터 넘겨주깅. 근데 뭔가 바꿔야 할것같아
    return p
...
def conductExperiment(p, alg):
    ...
    # else:
    #     alg.run(p)
    bestSolution = p.getSolution() #한번 동작시키면 솔루션, value 나오고, 그후 여러번 실험 할겨. numExp. 통계치~! bestSolution, 
    ...
    #numExp >=2    
    for i in range(1, numExp): #실험횟수를 numExp 변수로.
        # if 1 <= aType <= 4:
        alg.randomRestart(p)
        # else:
        #     alg.run(p)
